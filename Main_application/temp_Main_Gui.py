...
from GUI import Application_GUI_rc
...
class Abstract_Main_GUI(ABC):
    ...
    @abstractmethod
    def database_checker(self, title: str, message: str) -> None:
        pass
...
class Main_Gui(Abstract_Main_GUI):
    def __init__(self):
    ...
    def isdatabase_same_session(self) -> bool:
        ...
        logging.debug(f"{_timedelta_var_hour = }\n")
    ...
    def task_status_label_refresher(self) -> None:
        if self.app_main_window.isVisible():
            if len(self.app_main_window.main_window_ui.vendor_details_selection_combobox.currentText()) > 0:
                if self.app_main_window.main_window_ui.vendor_details_selection_combobox.currentText().strip() != 'Select Vendor':
