...
class App_main_application_first_window(QWidget):
    def __init__(self):
        ...
        self.username = (os.popen(cmd= 'cmd.exe /C "echo %username%"').read()).strip()
        self._file_dialog_lookup_folder = rf"C:\\Users\\{self.username}\\AppData\\Local\\CLI_Automation\\file_dialog_lookup.txt"
    ...
    def file_browser(self):
        ...
        if os.path.exists(self._file_dialog_lookup_folder):
            with open(self._file_dialog_lookup_folder, 'r') as _f:
                _lookup_folder = _f.readline()
                _f.close()
            del _f
            self.host_details_file, _ = QFileDialog.getOpenFileName(self, "Select Host Detail", _lookup_folder, "Excel File (*.xlsx);;All Files (*.*)", "Excel File (*.xlsx)")
        ...
        with open(self._file_dialog_lookup_folder, 'w') as _f:
            _f.write(
                os.path.dirname(os.path.dirname(self.host_details_file))
            )
            _f.close()
        del _f
