...
def main_func(dataframe : pd.DataFrame, ip_node: str) -> dict:
    ...
    logging.debug(f"Adding the system path for the Nokia main folder so that the NameError is not encountered in Nokia.Nokia_Section_Template_Checks.VPLS_1.same_vpls_id_found_error_message in VPLS-2 Section Checks")
    ...
    if(len(df_add) > 0):
        ...
        if(len(temp_df) > 0):
            ...
            result_dictionary[reason] = VPLS_1.same_vpls_id_found_error_message_generator(dataframe=temp_df, common_vpls_ids= temp_df['VPLS ID'].unique().astype(int))
            del temp_df
    ...
    logging.debug(f"Finding the setintersection for the df_add and df_delete for node_ip ({ip_node}) for VPLS-2 {np.intersect1d(df_add_unique_VPLS_IDs,df_delete_modify_VPLS_IDs).astype(int)}")
    ...
    if(set_intersection_between_Add_and_Modify_Delete_dfs.size > 0):
        ...
        from Nokia.Nokia_Section_Template_Checks.VPLS_1 import same_vpls_id_found_error_message_generator
